...
```
identityEncoding = "proof-of-deploy/workload-identity/v2\0"
                 || lp(clusterID) || lp(apiVersion) || lp(kind)
                 || lp(namespace) || lp(name)

workloadID = SHA-256(identityEncoding)
```
...
### Signed digest — nested, not flattened

The envelope embeds the identity encoding **as a length-prefixed unit** rather
than re-listing its fields:

```
signingPreimage = "proof-of-deploy/attestation/v2\0"
                || u16be(protocolVersion)
                || u32be(len(identityEncoding)) || identityEncoding
                || lp(uid)
                || configHash          // 32 raw bytes, fixed width, last

signedDigest = SHA-256(signingPreimage)
```

Nesting is better than the flat field list this ADR first sketched. There is one
definition of how an identity becomes bytes, shared by the on-chain key and the
signature, so the two can never drift apart; and adding a field to
`WorkloadIdentityV2` changes both by construction instead of requiring two
edits that must be kept in step.

### Incarnation as a fixed-width value

```
incarnation = uid == "" ? 32 zero bytes
                        : SHA-256("proof-of-deploy/incarnation/v2\0" || uid)
```

The signed digest binds the **raw** UID; `incarnation` is the fixed-width form
for a Solidity `bytes32`. It is a digest rather than the raw string because the
Kubernetes API types a UID as opaque: it is a UUID in every implementation
anyone runs, but nothing in the API guarantees a width, and an on-chain
fixed-width field must not depend on that. The all-zero value means "no
incarnation bound" and is produced only by an empty UID, so a zero field is
unambiguous rather than merely unset.
...
**Status:** implemented. `internal/attest/envelope_test.go` covers all five,
with `TestEnvelopeBindsEveryField` asserting that mutating any of protocol
version, clusterID, apiVersion, kind, namespace, name, UID, UID-removal or
config hash changes the digest — the property Decision 2 of ADR 0001 exists for.

The committed vectors in `testdata/_envelope_v2/reference.txt` are additionally
cross-checked by `reference.py`, an implementation written from the documented
layout rather than the Go source. It reproduces all five values byte-for-byte.
This matters because a vector generated by the code it pins proves determinism,
not correctness: if the encoder were wrong, the vector would be confidently
wrong too. Run `python3 reference.py --check` — it decodes the committed
vectors, re-derives them, and exits non-zero on any disagreement.
